...
async function addSyncButton() {
    ...
    newButton.onclick = function () {
        const appNumber = document.getElementById("appNumberInput").value;
        if (!appNumber) {
            alert("アプリ番号を入力してください");
            return;
        }
        // ボタンを灰色に変更し、クリックを無効化
        newButton.style.backgroundColor = "gray";
        newButton.style.pointerEvents = "none";
        syncAllRecords(appNumber); // 全レコード同期処理を呼び出す
    };
...
// 全レコード同期処理
async function syncAllRecords(appNumber) {
